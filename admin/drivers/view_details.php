    ...
    <div class="border border-dark px-2 py-2" id="print_out">
        ...
        <table class='table table-stripped px-4'>
            ...
            <tbody>
                ...
                $olist = $conn->query("SELECT i.*, o.code, o.offensename, ol.status, ol.date_created as Datetime
                           FROM `offense_items` i 
                           INNER JOIN `offenses` o ON i.offense_id = o.id 
                           INNER JOIN `offense_list1` ol ON i.driver_offense_id = ol.id 
                           WHERE ol.driver_id = '{$id}' 
                           ORDER BY unix_timestamp(i.date_created) ASC");
                ...
                // Initialize an array to track offense counts
                $offenseCounts = [];
                ...
                if ($olist->num_rows > 0):
                    ...
                    while ($row = $olist->fetch_assoc()):
                        ...
                        $offenseName = $row['offensename'];

                        // Increment the count for this offense
                        if (!isset($offenseCounts[$offenseName])) {
                            $offenseCounts[$offenseName] = 1;
                        } else {
                            $offenseCounts[$offenseName]++;
                        }

                        // Determine the offense number (1st, 2nd, etc.)
                        $offenseNumber = $offenseCounts[$offenseName];
                        $offenseLabel = $offenseNumber . ($offenseNumber == 1 ? "st" : ($offenseNumber == 2 ? "nd" : ($offenseNumber == 3 ? "rd" : "th"))) . " Offense";
                        ?>
                <tr>
                    <td><?php echo date("M d, Y h:i A", strtotime($row['Datetime'])); ?></td>
                    <td><?php echo htmlspecialchars($offenseName, ENT_QUOTES, 'UTF-8'); ?>
                        (<?php echo $offenseLabel; ?>)</td>
